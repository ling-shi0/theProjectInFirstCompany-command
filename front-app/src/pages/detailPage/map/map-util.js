...
export default class MapUtil {
    ...
    this.tileLayerConfig = Object.assign(
      {
        ...
        hmapStyleUrl: {
          white:
            'http://10.33.43.58:1709/hmap-server/hvtStyles/alarm-white/MapStyle.json',
          blue:
            'http://10.33.43.58:1709/hmap-server/hvtStyles/talarm-blue/MapStyle.json'
        }
      },
      tileLayerConfig
    );
  ...
  drawWktPolygon(name, polygons, callback, titleName = '陵城区') {
    let vectorLayer = this.map.getLayersByName(name)[0];
    if (!vectorLayer) {
      vectorLayer = new hmap.layer.VectorLayer(name, {
        enableHashCode: false
